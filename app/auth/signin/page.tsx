  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // Successfully logged in - check assessment status before redirecting
        // This ensures:
        // - New users (first-time login) → redirect to onboarding
        ...
        try {
          ...
          if (assessmentResponse.ok) {
            ...
            if (!assessmentData.completed) {
              ...
              console.log("[SIGNIN] Redirecting new user to onboarding");
    ...
    <div className="min-h-screen flex items-center justify-center bg-white p-4">
      <div className="w-full max-w-md">
        ...
        <div className="space-y-6">
          <h1 className="text-4xl font-bold text-center text-gray-900">Log in</h1>

          <form onSubmit={handleSubmit} className="space-y-6">
            {success && (
              <div className="p-4 text-sm text-green-600 bg-green-50 rounded-lg border border-green-200 flex items-center gap-2">
            ...
            <div className="space-y-2 relative">
              <Input
                ...
                required
                className="h-14 text-lg border-gray-300 rounded-lg focus:border-liberian-red focus:ring-liberian-red pr-20"
              />
              <Link
                href="/auth/forgot"
                className="absolute right-3 top-1/2 -translate-y-1/2 text-sm text-gray-500 hover:text-liberian-red"
              >
                FORGOT?
              </Link>
            </div>
            ...
            <Button
              type="submit"
              className="w-full h-14 text-lg font-semibold bg-liberian-blue hover:bg-liberian-blue/90 text-white rounded-lg"
              disabled={isLoading}
            >
          ...
          <div className="space-y-3">
            <Button
              ...
              onClick={() => {
                // Social auth will redirect to dashboard
                // App layout will automatically redirect to onboarding if user hasn't completed it
